...
Give every writable NeonCRM, Cvent, Impexium, PheedLoop and MagnetMail catalog object a primary key —
...
it would newly *enable* an unverified write path — the opposite of this change's purpose. (For
...
**PheedLoop** — `EventAttendance` write withdrawn. It is the event-scoped check-in *envelope*: all four
...
*"write-only-object: type of the uploadSuppressionList write payload (bulk suppress). No read
operation response contains a RecipientSuppressionList element."* (provenance: `scripts/wsdl.xml`).
