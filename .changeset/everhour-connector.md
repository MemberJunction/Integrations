---
'@memberjunction/connector-everhour': minor
---

New Everhour connector: team users, projects, per-project tasks and team-wide time records.
...
had *stripped* the prefix on the way in, with `id.slice(3)`, to make the remainder match an Asana gid,
...
(tasks 250). And `page` is documented on tasks and time but *not* on `/projects` — it does work there,
