...
(`@memberjunction/integration-connectors`) and the `MySQLExternalDriver` EDS driver, both of which publish in 5.46.0.
...
1. Add the `@memberjunction/*` peers to devDependencies (`^5.46.0`) and `npm install`; decide whether to bundle
   `@memberjunction/external-data-source-mysql` as a dependency (self-contained connector) vs. keep it a peer.
2. `mj sync push --dir metadata` against a fresh 5.46 DB (sqlLogging captures the seed SQL here), then
   `wrap-migration.mjs` → `V<YYYYMMDDHHMM>__mysql__Metadata.sql`, then `build-pg-migrations.mjs`.
