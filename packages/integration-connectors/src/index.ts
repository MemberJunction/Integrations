...
/**
 * Open App bootstrap entry point.
 *
 * Importing this module already ran every connector's `@RegisterClass(BaseIntegrationConnector, ...)`
 * decorator as a side effect, so the MJ ClassFactory can resolve any connector by its
 * `Integration.ClassName` at runtime. This no-op function exists only to give the Open App
 * loader an explicit `startupExport` symbol to name in `mj-app.json` (`packages.server[].startupExport`)
 * and to satisfy the required `dynamicPackages.server[].StartupExport` field — which is what
 * forces this module (and thus the registrations) to be imported at MJAPI boot.
 */
export function registerConnectors(): void {
  /* side-effect registration already happened on import; nothing to do */
}
