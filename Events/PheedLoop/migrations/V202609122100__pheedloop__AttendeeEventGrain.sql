...
UPDATE [__mj].[IntegrationObjectField]
   SET [IsUniqueKey] = 0,
       [Description] = N'Attendee code. Part of the key, NOT unique on its own: this object is fetched once per event, so an attendee who attended more than one event is returned once per event with the same code. Unique only with eventCode.'
...
IF NOT EXISTS (SELECT 1 FROM [__mj].[IntegrationObjectField]
  ...
  VALUES ('9C1F4B0E-27A6-4D3B-8E5A-6F0B71C4D982', '7FF3BC5E-EC39-4869-B6E5-4AF1C398C362', N'eventCode',
          N'The event this attendee row belongs to. APIPath is /events/{eventCode}/attendees/, so the engine tags every fetched record with the resolved parent id under the template var''s own name. Declared so the row can record WHICH event its is_checked_in and checkin_date describe.',
          N'string', 100, 1, 0, 0, 1, 0, N'Active', 0, N'Declared');
