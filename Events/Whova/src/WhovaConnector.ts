...
// package name, so a bare class-symbol key would never match in the catalog.
// The bare symbol stays registered as an alias because that is the ClassName
// this connector carried while it lived in the MemberJunction monorepo
// (packages/Integration/connectors), so any catalog row seeded from there
// keeps resolving.
@RegisterClass(BaseIntegrationConnector, '@memberjunction/connector-whova')
...
export class WhovaConnector extends BaseRESTIntegrationConnector {
    ...
    private async LoadCredentials(
        companyIntegration: MJCompanyIntegrationEntity,
        contextUser: UserInfo
    ): Promise<WhovaResolvedCredentials & { Token: string }> {
