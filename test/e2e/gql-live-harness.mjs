...
export async function phaseSetup({ gql, cfg }) {
    ...
    if (Array.isArray(cfg.entityMapInputs) && cfg.entityMapInputs.length) {
        ...
        // CHUNKED APPLYALL (E8): a single ApplyAll call materializing a huge catalog (e.g. Salesforce-shaped
        // orgs with hundreds of objects, applyScope:'full') is infeasible in one request — it can time out or
        // fail partway through, discarding progress on every object in that call. Batch SourceObjects into
        // fixed-size chunks and accumulate EntityMapsCreated/Warnings/Steps across calls, so materialization
        // makes durable per-batch progress instead of all-or-nothing. Small sets (the common 'scoped' default)
        // fit in one batch and behave exactly as before — this only changes behavior at scale.
        // ONE-BATCH DEFAULT: chunking into groups of 40 forced ~43 ApplyAll calls for a 1701-object
        // Salesforce catalog — and each ApplyAll runs a full whole-DB CodeGen (~50-150s, growing with
        // entity count), so 43 chunks = ~2 hours of repeated CodeGen. A single ApplyAll = ONE CodeGen.
        // Default to one batch; set E2E_APPLYALL_CHUNK_SIZE to re-enable per-batch durability at scale.
        const APPLY_CHUNK_SIZE = Number(process.env.E2E_APPLYALL_CHUNK_SIZE || 100000);
        const chunks = [];
        for (let i = 0; i < applyObjectNames.length; i += APPLY_CHUNK_SIZE) chunks.push(applyObjectNames.slice(i, i + APPLY_CHUNK_SIZE));
        if (!chunks.length) chunks.push([]);
        const allWarnings = [], allSteps = [], allMapsCreated = [];
        for (let i = 0; i < chunks.length; i++) {
            const chunk = chunks[i];
            const res = expectGqlSuccess('ApplyAll', (await gql(GQL.applyAll, {
                input: {
                    CompanyIntegrationID: ciid,
                    SourceObjects: chunk.map(name => ({ SourceObjectName: name })),
            ...
            allSteps.push(...(res.Steps ?? []).map(s => ({ ...s, Message: `[batch ${i + 1}/${chunks.length}] ${s.Message ?? ''}` })));
            allMapsCreated.push(...(res.EntityMapsCreated ?? []));
        }
        applied = { Warnings: allWarnings, Steps: allSteps, EntityMapsCreated: allMapsCreated };
        maps = allMapsCreated.map(m => ({
