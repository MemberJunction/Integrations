...
SET Configuration =
        JSON_MODIFY(
            JSON_MODIFY(
                Configuration,
                '$.idWindowScan',
                JSON_QUERY(N'{"wsFunction":"core_user_get_users_by_field","field":"id","windowSize":25,"windowsPerCall":2,"maxConsecutiveEmptyWindows":40,"budgetMs":20000,"maxBisectSplitsPerCall":8}')
            ),
            '$.idWindowScanRationale',
            N'core_user_get_users declares NO pagination params and the vendor docs state it "could [be] very slow or timeout" without narrow criteria (and that the % wildcard "may be considerably slower"). The defaultArgs email=% bulk list is therefore a documented anti-pattern: against a real-sized site it never returns inside the engine''s FetchChangesMs budget and the object syncs ZERO rows behind a green run (observed live: 3x 30s timeouts, 0 users). Reads route through the bounded id-window scan instead - an indexed PK lookup via the documented bulk reader. defaultArgs is retained only as the legacy fallback if idWindowScan is removed.'
        )
WHERE Name = 'Users'
  AND ISJSON(Configuration) = 1
  AND JSON_VALUE(Configuration, '$.wsfunction') = 'core_user_get_users'
  AND JSON_QUERY(Configuration, '$.idWindowScan') IS NULL
