...
UPDATE __mj."IntegrationObject"
SET "Status" = 'Disabled',
    "Description" = 'Disabled: ''binputawayworksheet'' is not a NetSuite record type — SuiteQL rejects it with HTTP 400 "Invalid search type: binputawayworksheet" (INVALID_PARAMETER) on every account, so this object could never sync. The real record type is ''binworksheet'' (NetSuite''s SuiteScript record.Type id), which DiscoverObjects surfaces from the account''s live metadata-catalog.'
...
UPDATE __mj."IntegrationObject"
SET "Status" = 'Disabled',
    "Description" = 'Disabled: ''requisition'' is not a NetSuite record type — SuiteQL rejects it with HTTP 400 "Invalid search type: requisition" (INVALID_PARAMETER) on every account, so this object could never sync. The real record type is ''purchaserequisition'' (NetSuite''s SuiteScript record.Type id), which DiscoverObjects surfaces from the account''s live metadata-catalog.'
...
UPDATE __mj."IntegrationObject"
SET "Status" = 'Disabled',
    "Description" = 'Disabled: ''weeklytimesheet'' is not a NetSuite record type — SuiteQL rejects it with HTTP 400 "Invalid search type: weeklytimesheet" (INVALID_PARAMETER) on every account, so this object could never sync. The real record type is ''timesheet'' (NetSuite''s SuiteScript record.Type id), which DiscoverObjects surfaces from the account''s live metadata-catalog.'
