...
export class SageIntacctConnector extends BaseIntegrationConnector {
     ...
     *
     * `super.IntrospectSchema` yields the Declared catalog — for Sage Intacct that is the live `inspect`
     * describe (no measured widths). Per object we call MJ's `DiscoverFieldsViaFetch` (MJ's own read-path
     * sampler — measures real widths, surfaces custom columns) and the shared PURE
     * `mergeDeclaredWithSampledFields` unions the two by field name (never-shrink `max()` width; append
     * MJ-discovered custom columns). MJ owns everything else.
     *
