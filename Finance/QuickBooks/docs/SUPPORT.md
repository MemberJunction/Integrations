...
> **Evidence tier:** 🟡 Honest-NA (untested for a documented, non-defect reason)  ·  **Last verified:** 2026-08-12  ·  **Proof DB(s):** —
...
**39 objects** declared across **2683 fields** (source: `metadata/integration/.quickbooks.integration.json`). 35 declare a write path; 4 are read-only (pull). 35 support incremental sync.
...
| Employee | ✓ | `CU` | ✓ |
| Estimate | ✓ | `CUD` | ✓ |
| ExchangeRate | ✓ | — (read-only) | ✓ |
| InventoryAdjustment | ✓ | `CU` | ✓ |
| Invoice | ✓ | `CUD` | ✓ |
| Item | ✓ | `CU` | ✓ |
| JournalCode | ✓ | `CU` | ✓ |
| JournalEntry | ✓ | `CUD` | ✓ |
...
| Purchase | ✓ | `CUD` | ✓ |
| PurchaseOrder | ✓ | `CUD` | ✓ |
| RecurringTransaction | ✓ | — (read-only) | — |
| RefundReceipt | ✓ | `CUD` | ✓ |
| ReimburseCharge | ✓ | `CU` | ✓ |
| SalesReceipt | ✓ | `CUD` | ✓ |

_First 30 of 39 objects shown, alphabetically. The full catalog is the metadata file cited above._
...
- **Coverage:** 0 of 39 declared objects have proven rows.
