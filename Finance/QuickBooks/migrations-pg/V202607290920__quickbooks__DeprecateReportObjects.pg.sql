...
UPDATE "__mj"."IntegrationObject"
SET "Status"      = 'Deprecated',
    "Description" = 'Profit and Loss (income statement) report. Deprecated: not a syncable record set. The QBO Reports API returns one report envelope (Header/Columns/Rows) rather than a collection of identified records, so this catalog row carries a single Rows blob and no primary key — MJ therefore builds no entity for it. QuickBooksConnector has no report code path at all: FetchChanges and ListRecords both issue SELECT * FROM <ObjectName> against the /query endpoint (reports are not queryable entities) and key every record on r[''Id''], which a report has no equivalent of; the declared reports APIPath is never read. The connector''s own QUICKBOOKS_OBJECTS constant — the source for DiscoverObjects — contains no Report.* entry, so rediscovery will not reactivate this row.'
...
UPDATE "__mj"."IntegrationObject"
SET "Status"      = 'Deprecated',
    "Description" = 'Detailed Profit and Loss report with transaction drill-down. Deprecated: not a syncable record set. The QBO Reports API returns one report envelope (Header/Columns/Rows) rather than a collection of identified records, so this catalog row carries a single Rows blob and no primary key — MJ therefore builds no entity for it. QuickBooksConnector has no report code path at all: FetchChanges and ListRecords both issue SELECT * FROM <ObjectName> against the /query endpoint (reports are not queryable entities) and key every record on r[''Id''], which a report has no equivalent of; the declared reports APIPath is never read. The connector''s own QUICKBOOKS_OBJECTS constant — the source for DiscoverObjects — contains no Report.* entry, so rediscovery will not reactivate this row.'
...
UPDATE "__mj"."IntegrationObject"
SET "Status"      = 'Deprecated',
    "Description" = 'Trial Balance listing all account debits and credits. Deprecated: not a syncable record set. The QBO Reports API returns one report envelope (Header/Columns/Rows) rather than a collection of identified records, so this catalog row carries a single Rows blob and no primary key — MJ therefore builds no entity for it. QuickBooksConnector has no report code path at all: FetchChanges and ListRecords both issue SELECT * FROM <ObjectName> against the /query endpoint (reports are not queryable entities) and key every record on r[''Id''], which a report has no equivalent of; the declared reports APIPath is never read. The connector''s own QUICKBOOKS_OBJECTS constant — the source for DiscoverObjects — contains no Report.* entry, so rediscovery will not reactivate this row.'
...
UPDATE "__mj"."IntegrationObject"
SET "Status"      = 'Deprecated',
    "Description" = 'General Ledger report with full transaction history per account. Deprecated: not a syncable record set. The QBO Reports API returns one report envelope (Header/Columns/Rows) rather than a collection of identified records, so this catalog row carries a single Rows blob and no primary key — MJ therefore builds no entity for it. QuickBooksConnector has no report code path at all: FetchChanges and ListRecords both issue SELECT * FROM <ObjectName> against the /query endpoint (reports are not queryable entities) and key every record on r[''Id''], which a report has no equivalent of; the declared reports APIPath is never read. The connector''s own QUICKBOOKS_OBJECTS constant — the source for DiscoverObjects — contains no Report.* entry, so rediscovery will not reactivate this row.'
...
UPDATE "__mj"."IntegrationObject"
SET "Status"      = 'Deprecated',
    "Description" = 'Cash Flow statement report. Deprecated: not a syncable record set. The QBO Reports API returns one report envelope (Header/Columns/Rows) rather than a collection of identified records, so this catalog row carries a single Rows blob and no primary key — MJ therefore builds no entity for it. QuickBooksConnector has no report code path at all: FetchChanges and ListRecords both issue SELECT * FROM <ObjectName> against the /query endpoint (reports are not queryable entities) and key every record on r[''Id''], which a report has no equivalent of; the declared reports APIPath is never read. The connector''s own QUICKBOOKS_OBJECTS constant — the source for DiscoverObjects — contains no Report.* entry, so rediscovery will not reactivate this row.'
...
UPDATE "__mj"."IntegrationObject"
SET "Status"      = 'Deprecated',
    "Description" = 'Customer open balance summary. Deprecated: not a syncable record set. The QBO Reports API returns one report envelope (Header/Columns/Rows) rather than a collection of identified records, so this catalog row carries a single Rows blob and no primary key — MJ therefore builds no entity for it. QuickBooksConnector has no report code path at all: FetchChanges and ListRecords both issue SELECT * FROM <ObjectName> against the /query endpoint (reports are not queryable entities) and key every record on r[''Id''], which a report has no equivalent of; the declared reports APIPath is never read. The connector''s own QUICKBOOKS_OBJECTS constant — the source for DiscoverObjects — contains no Report.* entry, so rediscovery will not reactivate this row.'
...
UPDATE "__mj"."IntegrationObject"
SET "Status"      = 'Deprecated',
    "Description" = 'Customer open balance with per-transaction detail. Deprecated: not a syncable record set. The QBO Reports API returns one report envelope (Header/Columns/Rows) rather than a collection of identified records, so this catalog row carries a single Rows blob and no primary key — MJ therefore builds no entity for it. QuickBooksConnector has no report code path at all: FetchChanges and ListRecords both issue SELECT * FROM <ObjectName> against the /query endpoint (reports are not queryable entities) and key every record on r[''Id''], which a report has no equivalent of; the declared reports APIPath is never read. The connector''s own QUICKBOOKS_OBJECTS constant — the source for DiscoverObjects — contains no Report.* entry, so rediscovery will not reactivate this row.'
...
UPDATE "__mj"."IntegrationObject"
SET "Status"      = 'Deprecated',
    "Description" = 'Sales by Customer summary report. Deprecated: not a syncable record set. The QBO Reports API returns one report envelope (Header/Columns/Rows) rather than a collection of identified records, so this catalog row carries a single Rows blob and no primary key — MJ therefore builds no entity for it. QuickBooksConnector has no report code path at all: FetchChanges and ListRecords both issue SELECT * FROM <ObjectName> against the /query endpoint (reports are not queryable entities) and key every record on r[''Id''], which a report has no equivalent of; the declared reports APIPath is never read. The connector''s own QUICKBOOKS_OBJECTS constant — the source for DiscoverObjects — contains no Report.* entry, so rediscovery will not reactivate this row.'
...
UPDATE "__mj"."IntegrationObject"
SET "Status"      = 'Deprecated',
    "Description" = 'Sales by Customer with per-transaction detail. Deprecated: not a syncable record set. The QBO Reports API returns one report envelope (Header/Columns/Rows) rather than a collection of identified records, so this catalog row carries a single Rows blob and no primary key — MJ therefore builds no entity for it. QuickBooksConnector has no report code path at all: FetchChanges and ListRecords both issue SELECT * FROM <ObjectName> against the /query endpoint (reports are not queryable entities) and key every record on r[''Id''], which a report has no equivalent of; the declared reports APIPath is never read. The connector''s own QUICKBOOKS_OBJECTS constant — the source for DiscoverObjects — contains no Report.* entry, so rediscovery will not reactivate this row.'
...
UPDATE "__mj"."IntegrationObject"
SET "Status"      = 'Deprecated',
    "Description" = 'Vendor open balance summary. Deprecated: not a syncable record set. The QBO Reports API returns one report envelope (Header/Columns/Rows) rather than a collection of identified records, so this catalog row carries a single Rows blob and no primary key — MJ therefore builds no entity for it. QuickBooksConnector has no report code path at all: FetchChanges and ListRecords both issue SELECT * FROM <ObjectName> against the /query endpoint (reports are not queryable entities) and key every record on r[''Id''], which a report has no equivalent of; the declared reports APIPath is never read. The connector''s own QUICKBOOKS_OBJECTS constant — the source for DiscoverObjects — contains no Report.* entry, so rediscovery will not reactivate this row.'
...
UPDATE "__mj"."IntegrationObject"
SET "Status"      = 'Deprecated',
    "Description" = 'Vendor open balance with per-transaction detail. Deprecated: not a syncable record set. The QBO Reports API returns one report envelope (Header/Columns/Rows) rather than a collection of identified records, so this catalog row carries a single Rows blob and no primary key — MJ therefore builds no entity for it. QuickBooksConnector has no report code path at all: FetchChanges and ListRecords both issue SELECT * FROM <ObjectName> against the /query endpoint (reports are not queryable entities) and key every record on r[''Id''], which a report has no equivalent of; the declared reports APIPath is never read. The connector''s own QUICKBOOKS_OBJECTS constant — the source for DiscoverObjects — contains no Report.* entry, so rediscovery will not reactivate this row.'
...
UPDATE "__mj"."IntegrationObject"
SET "Status"      = 'Deprecated',
    "Description" = 'Expenses by Vendor summary. Deprecated: not a syncable record set. The QBO Reports API returns one report envelope (Header/Columns/Rows) rather than a collection of identified records, so this catalog row carries a single Rows blob and no primary key — MJ therefore builds no entity for it. QuickBooksConnector has no report code path at all: FetchChanges and ListRecords both issue SELECT * FROM <ObjectName> against the /query endpoint (reports are not queryable entities) and key every record on r[''Id''], which a report has no equivalent of; the declared reports APIPath is never read. The connector''s own QUICKBOOKS_OBJECTS constant — the source for DiscoverObjects — contains no Report.* entry, so rediscovery will not reactivate this row.'
...
UPDATE "__mj"."IntegrationObject"
SET "Status"      = 'Deprecated',
    "Description" = 'Aged Payables summary (vendor aging). Deprecated: not a syncable record set. The QBO Reports API returns one report envelope (Header/Columns/Rows) rather than a collection of identified records, so this catalog row carries a single Rows blob and no primary key — MJ therefore builds no entity for it. QuickBooksConnector has no report code path at all: FetchChanges and ListRecords both issue SELECT * FROM <ObjectName> against the /query endpoint (reports are not queryable entities) and key every record on r[''Id''], which a report has no equivalent of; the declared reports APIPath is never read. The connector''s own QUICKBOOKS_OBJECTS constant — the source for DiscoverObjects — contains no Report.* entry, so rediscovery will not reactivate this row.'
...
UPDATE "__mj"."IntegrationObject"
SET "Status"      = 'Deprecated',
    "Description" = 'Aged Payables with per-bill detail. Deprecated: not a syncable record set. The QBO Reports API returns one report envelope (Header/Columns/Rows) rather than a collection of identified records, so this catalog row carries a single Rows blob and no primary key — MJ therefore builds no entity for it. QuickBooksConnector has no report code path at all: FetchChanges and ListRecords both issue SELECT * FROM <ObjectName> against the /query endpoint (reports are not queryable entities) and key every record on r[''Id''], which a report has no equivalent of; the declared reports APIPath is never read. The connector''s own QUICKBOOKS_OBJECTS constant — the source for DiscoverObjects — contains no Report.* entry, so rediscovery will not reactivate this row.'
...
UPDATE "__mj"."IntegrationObject"
SET "Status"      = 'Deprecated',
    "Description" = 'Inventory valuation summary. Deprecated: not a syncable record set. The QBO Reports API returns one report envelope (Header/Columns/Rows) rather than a collection of identified records, so this catalog row carries a single Rows blob and no primary key — MJ therefore builds no entity for it. QuickBooksConnector has no report code path at all: FetchChanges and ListRecords both issue SELECT * FROM <ObjectName> against the /query endpoint (reports are not queryable entities) and key every record on r[''Id''], which a report has no equivalent of; the declared reports APIPath is never read. The connector''s own QUICKBOOKS_OBJECTS constant — the source for DiscoverObjects — contains no Report.* entry, so rediscovery will not reactivate this row.'
...
UPDATE "__mj"."IntegrationObject"
SET "Status"      = 'Deprecated',
    "Description" = 'Sales by Product/Service summary. Deprecated: not a syncable record set. The QBO Reports API returns one report envelope (Header/Columns/Rows) rather than a collection of identified records, so this catalog row carries a single Rows blob and no primary key — MJ therefore builds no entity for it. QuickBooksConnector has no report code path at all: FetchChanges and ListRecords both issue SELECT * FROM <ObjectName> against the /query endpoint (reports are not queryable entities) and key every record on r[''Id''], which a report has no equivalent of; the declared reports APIPath is never read. The connector''s own QUICKBOOKS_OBJECTS constant — the source for DiscoverObjects — contains no Report.* entry, so rediscovery will not reactivate this row.'
...
--    Each Report.* row declares exactly ONE field, "Rows" (TEXT, IsReadOnly=1), described in
