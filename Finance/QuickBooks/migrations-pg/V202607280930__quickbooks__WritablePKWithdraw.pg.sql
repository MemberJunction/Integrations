...
-- HAND-CORRECTED after conversion: `mj migrate convert` (5.43.0, and still 5.49.0) coerces only the
-- FIRST boolean column in a multi-column SET list, so it emitted `"SupportsCreate" = 0` and
-- `"SupportsUpdate" = 0` against BOOLEAN columns — which fails on apply with
--     column "SupportsCreate" is of type boolean but expression is of type integer
-- while the SQL Server side applies fine. Both are set to FALSE below. Reproduced on a minimal
-- two-boolean UPDATE, so it is the converter, not this file. Regeneration will not clobber this:
-- build-pg-migrations.mjs only converts migrations whose .pg.sql counterpart is missing.
