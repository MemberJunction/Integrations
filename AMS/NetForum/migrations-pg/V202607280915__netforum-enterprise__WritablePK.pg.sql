...
-- "SupportsDelete", so the converter left those as the SQL Server bit literals 0/1. PG rejects them
-- at APPLY time with 'column "SupportsCreate" is of type boolean but expression is of type integer'.
-- The 0s below are rewritten to FALSE by hand. Fixed upstream in MemberJunction/MJ #3294; once that
-- ships in a consumed release the converter emits FALSE itself and this note can go.
