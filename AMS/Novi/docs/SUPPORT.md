...
**32 objects** declared across **387 fields** (source: `metadata/integration/.novi-ams.integration.json`). 13 declare a write path; 19 are read-only (pull). 9 support incremental sync.
...
| Article | ✓ | `CUD` | ✓ |
| Attendee | ✓ | — (read-only) | ✓ |
| BlogCategory | ✓ | — (read-only) | — |
| Class | ✓ | — (read-only) | — |
| Committee | ✓ | — (read-only) | ✓ |
| CommitteeMember | ✓ | `CUD` | — |
| CommitteeRole | ✓ | — (read-only) | — |
| CreditType | ✓ | — (read-only) | — |
| CustomActivityType | ✓ | `CU` | — |
| CustomerCustomField | ✓ | `U` | — |
| CustomField | ✓ | — (read-only) | — |
| Event | ✓ | `CU` | — |
| EventCategory | ✓ | — (read-only) | — |
| Folder | ✓ | — (read-only) | — |
| Group | ✓ | — (read-only) | — |
...
| Item | ✓ | — (read-only) | — |
| Member | ✓ | `CU` | ✓ |
| MembershipActivity | ✓ | — (read-only) | ✓ |
| MemberType | ✓ | — (read-only) | — |
| NpsSurvey | ✓ | `CU` | ✓ |
| Order | ✓ | `CU` | ✓ |
| OrderItem | ✓ | — (read-only) | — |
| Product | ✓ | — (read-only) | — |
| ProductPurchase | ✓ | — (read-only) | ✓ |
| Registration | ✓ | `CU` | — |
| StaticPage | ✓ | `CUD` | — |
...
- **Coverage:** 0 of 32 declared objects have proven rows.
