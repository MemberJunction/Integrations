...
**216 objects** declared across **2275 fields** (source: `metadata/integration/.imis.integration.json`). 185 declare a write path; 31 are read-only (pull). 0 support incremental sync.

| Object | Pull | Push (C/U/D) | Incremental |
|---|---|---|---|
| AdAdjustment | ✓ | — (read-only) | — |
| AddressFormat | ✓ | — (read-only) | — |
| AddressPurpose | ✓ | — (read-only) | — |
| AdIssueDate | ✓ | — (read-only) | — |
| AdSize | ✓ | — (read-only) | — |
| AdvertiserAgencyMap | ✓ | — (read-only) | — |
| AdvertiserRepTerritoryMap | ✓ | — (read-only) | — |
| ApiDefinition | ✓ | — (read-only) | — |
| AppealSummary | ✓ | — (read-only) | — |
| AutoPayAccount | ✓ | — (read-only) | — |
| AutoPayInstruction | ✓ | — (read-only) | — |
| AutoPayProcessor | ✓ | — (read-only) | — |
| BatchSummary | ✓ | — (read-only) | — |
| BillingIssueNotificationLog | ✓ | — (read-only) | — |
| BillingIssueNotificationLogDetail | ✓ | — (read-only) | — |
| BillingLog | ✓ | — (read-only) | — |
| BillingLogItem | ✓ | — (read-only) | — |
| BOEntityDefinition | ✓ | — (read-only) | — |
| CampaignSummary | ✓ | — (read-only) | — |
| Cart | ✓ | — (read-only) | — |
| CertificationProgram | ✓ | — (read-only) | — |
| CertificationProgramComponent | ✓ | — (read-only) | — |
| CertificationProgramGroup | ✓ | — (read-only) | — |
| CertificationProgramRegistration | ✓ | — (read-only) | — |
| CertificationProgramSummary | ✓ | — (read-only) | — |
| ChapterZipCode | ✓ | — (read-only) | — |
| ComboOrder | ✓ | — (read-only) | — |
| CommitteeMinutes | ✓ | — (read-only) | — |
| Communication | ✓ | — (read-only) | — |
| CommunicationType | ✓ | — (read-only) | — |

_First 30 of 216 objects shown, alphabetically. The full catalog is the metadata file cited above._
...
- **Coverage:** 0 of 216 declared objects have proven rows.
