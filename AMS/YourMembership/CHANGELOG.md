# @memberjunction/connector-yourmembership

## 1.1.2

### Patch Changes

- 048dda4: Give every writable YourMembership catalog object a primary key — or withdraw the write it cannot
  ...
  **Withdrawn (41)**, in four groups:
  ...
  - **(a) RPC command payloads — a verb, not a record (21).** `ConvertToMemberRequest`,
    ...
    no item route, and its declared fields are the call's _arguments_ plus its _outcome_ — e.g.
...
- 533fb7a: ClassName follows the catalog convention (== npm package name) so instance discovery matches; legacy 'YourMembershipConnector' key stays registered and a delta migration fixes existing tenants' Integration rows.
