...
UPDATE [__mj].IntegrationObject
SET Description = N'Return a Custom Page. Primary key: PageID, present on all four declared routes (GET/PUT/POST/DELETE /Ams/{ClientID}/CustomPages) and echoed by the response as PageIDUsedByAnotherPage.'
...
UPDATE [__mj].IntegrationObject
SET Description = N'Get a list of organization posts. Primary key: PostId, the required path token of the item routes PUT|DELETE /Ams/{ClientID}/OrganizationPosts/{PostId}.'
...
UPDATE [__mj].IntegrationObject
SET SupportsWrite = 0,
    Description   = N'Update the event session attendance Command payload ("Update the event session attendance"): arguments (RegistrantID, SessionID, AttendedSession) plus the outcome (Success, Message). Write withdrawn: a writable object with no primary key derives a keyless entity whose saves cannot carry a record identifier. Reads are unaffected.'
WHERE ID = '1D51C876-12F5-464F-A413-E0122992C0BF';

-- ── withdraw 4. FilesUpload (a) — POST /Ams/{ClientID}/FilesUpload ──
UPDATE [__mj].IntegrationObject
SET SupportsWrite = 0,
    Description   = N'Upload community files. Upload command ("Upload community files"): a POST-only route with no collection, no item route and no returned identifier. Write withdrawn: a writable object with no primary key derives a keyless entity whose saves cannot carry a record identifier. Reads are unaffected.'
WHERE ID = 'F894FC9D-C87B-47FB-A1D8-6B1048B594F5';

-- ── withdraw 5. HtmlSanitization (a) — POST /Ams/{ClientID}/HtmlSanitization ──
UPDATE [__mj].IntegrationObject
SET SupportsWrite = 0,
    Description   = N'Return a sanitized Html A pure function, not a record: it takes Body and returns SanitizedBody. Nothing is stored, so nothing can be keyed. Write withdrawn: a writable object with no primary key derives a keyless entity whose saves cannot carry a record identifier. Reads are unaffected.'
...
UPDATE [__mj].IntegrationObject
SET SupportsWrite = 0,
    Description   = N'Upload emails and personal data to Informz for Dues by SearchId. Bulk-upload command: the two fields are its arguments (GroupName, SearchId). Write withdrawn: a writable object with no primary key derives a keyless entity whose saves cannot carry a record identifier. Reads are unaffected.'
...
UPDATE [__mj].IntegrationObject
SET SupportsWrite = 0,
    Description   = N'Upload emails and personal data to Informz for Orders by SearchId. Bulk-upload command: the two fields are its arguments (GroupName, SearchId). Write withdrawn: a writable object with no primary key derives a keyless entity whose saves cannot carry a record identifier. Reads are unaffected.'
...
UPDATE [__mj].IntegrationObject
SET SupportsWrite = 0,
    Description   = N'Upload emails and personal data to Informz for Reports. Bulk-upload command: the two fields are its arguments (GroupName, SearchId). Write withdrawn: a writable object with no primary key derives a keyless entity whose saves cannot carry a record identifier. Reads are unaffected.'
WHERE ID = 'AB868636-3094-48C3-8602-3FBFF98E604E';

-- ── withdraw 13. InformzFindGroupRequest (a) — POST /Ams/{ClientID}/InformzFindGroup ──
UPDATE [__mj].IntegrationObject
SET SupportsWrite = 0,
    Description   = N'Return whether the given group already exists. An existence query ("Return whether the given group already exists") shaped as a POST: GroupName in, Exist out. Write withdrawn: a writable object with no primary key derives a keyless entity whose saves cannot carry a record identifier. Reads are unaffected.'
WHERE ID = '8AD2C811-0B3A-4339-A26F-E83A2DDE66B1';

-- ── withdraw 14. InvoicePayments (a) — POST /Ams/{ClientID}/InvoicePayments ──
UPDATE [__mj].IntegrationObject
SET SupportsWrite = 0,
    Description   = N'Apply a payment to an invoice. Command payload ("Apply a payment to an invoice"): it names the InvoiceNo it applies to, returns no payment identifier, and has no collection or item route. Write withdrawn: a writable object with no primary key derives a keyless entity whose saves cannot carry a record identifier. Reads are unaffected.'
WHERE ID = '752162BD-EFC2-4459-A225-E28668755B4A';

-- ── withdraw 15. PeopleBulkDetachRequest (a) — POST /Ams/{ClientID}/PeopleBulkDetach ──
UPDATE [__mj].IntegrationObject
SET SupportsWrite = 0,
    Description   = N'Detach multiple sub accounts from master account Bulk command ("Detach multiple sub accounts from master account"): it carries a list (SubAccountIds) and the outcome (Status, Flag) — many targets, one call, no record. Write withdrawn: a writable object with no primary key derives a keyless entity whose saves cannot carry a record identifier. Reads are unaffected.'
...
UPDATE [__mj].IntegrationObject
SET SupportsWrite = 0,
    Description   = N'Returns a list of Time zones. A fetch-and-transform function, not a record: Url in, SiteContent out. Nothing is stored. Write withdrawn: a writable object with no primary key derives a keyless entity whose saves cannot carry a record identifier. Reads are unaffected.'
...
UPDATE [__mj].IntegrationObject
SET SupportsWrite = 0,
    Description   = N'Update store products status in bulk. Bulk command ("Update store products status in bulk"): one call carries a list of ProductIDs plus the Status to apply to all of them. Write withdrawn: a writable object with no primary key derives a keyless entity whose saves cannot carry a record identifier. Reads are unaffected.'
...
UPDATE [__mj].IntegrationObject
SET SupportsWrite = 0,
    Description   = N'Update all store products status in bulk. Bulk command ("Update all store products status in bulk"): its fields are filters (NameFilter, CategoryId, StatusFilter) plus the Status to apply. Write withdrawn: a writable object with no primary key derives a keyless entity whose saves cannot carry a record identifier. Reads are unaffected.'
WHERE ID = '02A38EB0-C9EA-4586-9FC0-73AF6FF0C592';

-- ── withdraw 20. StoreProductSequence (a) — PUT /Ams/{ClientID}/StoreProductSequence ──
UPDATE [__mj].IntegrationObject
SET SupportsWrite = 0,
    Description   = N'Set the sequence for a store product. Bulk command ("Set the sequence for a store product"): one call carries a whole ProductList for a category. Write withdrawn: a writable object with no primary key derives a keyless entity whose saves cannot carry a record identifier. Reads are unaffected.'
WHERE ID = 'D01C4CC7-0DEE-4AF7-ACD2-CDC712C508BF';

-- ── withdraw 21. StoreProductUpdate (a) — PUT /Ams/{ClientID}/StoreProductUpdate ──
UPDATE [__mj].IntegrationObject
SET SupportsWrite = 0,
    Description   = N'Update store products. Bulk command ("Update store products"): its single field, ProductIdStockLevelCombo, is a packed list of product/stock-level pairs. Write withdrawn: a writable object with no primary key derives a keyless entity whose saves cannot carry a record identifier. Reads are unaffected.'
...
UPDATE [__mj].IntegrationObject
SET SupportsWrite = 0,
    Description   = N'Cancel single and multiple invoice of donation. A member-scoped cancel command; the connector writes to /Ams/{ClientID}/{ObjectName}, which cannot express the /Member/{MemberID}/ segment. Write withdrawn: a writable object with no primary key derives a keyless entity whose saves cannot carry a record identifier. Reads are unaffected.'
WHERE ID = '93E92665-E33A-4520-88EA-65EDB3E2A523';

-- ── withdraw 23. EventAlias (b) — GET|POST|DELETE /Ams/{ClientID}/Event/{EventId}/Alias ──
UPDATE [__mj].IntegrationObject
SET SupportsWrite = 0,
    Description   = N'Return an event''s alias. An event-scoped singleton (one alias per event) addressed under /Event/{EventId}/; the connector cannot express the parent segment. Write withdrawn: a writable object with no primary key derives a keyless entity whose saves cannot carry a record identifier. Reads are unaffected.'
...
UPDATE [__mj].IntegrationObject
SET SupportsWrite = 0,
    Description   = N'Return a list of message folders for a member. A genuine member-scoped record keyed on FolderId, but addressed under /Member/{MemberID}/, which the connector’s generic write URL cannot express. Write withdrawn: a writable object with no primary key derives a keyless entity whose saves cannot carry a record identifier. Reads are unaffected.'
...
UPDATE [__mj].IntegrationObject
SET SupportsWrite = 0,
    Description   = N'Subscribe a user/device for Push Notification. A member-scoped device-subscribe command ("Subscribe a user/device for Push Notification"), addressed under a parent segment the connector cannot express. Write withdrawn: a writable object with no primary key derives a keyless entity whose saves cannot carry a record identifier. Reads are unaffected.'
...
UPDATE [__mj].IntegrationObject
SET SupportsWrite = 0,
    Description   = N'Update the details of notification. A genuine record keyed on NotificationID, but the vendor addresses it under /Notification/, while the connector writes to /{ObjectName} — here the literal segment "NotificationUpdate", which is not a route. Write withdrawn: a writable object with no primary key derives a keyless entity whose saves cannot carry a record identifier. Reads are unaffected.'
...
UPDATE [__mj].IntegrationObject
SET SupportsWrite = 0,
    Description   = N'Add a comment to a photo. A genuine record keyed on CommentId, but doubly parent-scoped (/Member/{MemberID}/Photos/{PhotoId}/), which the connector cannot express. Write withdrawn: a writable object with no primary key derives a keyless entity whose saves cannot carry a record identifier. Reads are unaffected.'
...
UPDATE [__mj].IntegrationObject
SET SupportsWrite = 0,
    Description   = N'Send the Test Email Notification. A send command ("Send the Test Email Notification") scoped under a notification; nothing is created, and the parent segment is not expressible. Write withdrawn: a writable object with no primary key derives a keyless entity whose saves cannot carry a record identifier. Reads are unaffected.'
WHERE ID = '4F848F28-74BC-4677-B0C8-4EF0E40E979D';

-- ── withdraw 32. WallPostFirst (b) — POST /Ams/{ClientID}/Member/{MemberID}/WallPostFirst ──
UPDATE [__mj].IntegrationObject
SET SupportsWrite = 0,
    Description   = N'Add a member''s first Post to the wall. A member-scoped POST-only command ("Add a member’s first Post to the wall") under a parent segment the connector cannot express. Write withdrawn: a writable object with no primary key derives a keyless entity whose saves cannot carry a record identifier. Reads are unaffected.'
...
UPDATE [__mj].IntegrationObject
SET SupportsWrite = 0,
    Description   = N'Add a new post to the wall. A genuine record keyed on PostId, but member-scoped under /Member/{MemberID}/, which the connector’s generic write URL cannot express. Write withdrawn: a writable object with no primary key derives a keyless entity whose saves cannot carry a record identifier. Reads are unaffected.'
WHERE ID = '9366A988-3776-434A-AF4F-530EF316F868';

-- ── withdraw 34. BrandingConfig (c) — GET|PUT|DELETE /Ams/{ClientID}/BrandingConfig ──
UPDATE [__mj].IntegrationObject
SET SupportsWrite = 0,
    Description   = N'Retrieve branding configuration for the given device. The client’s single branding configuration, addressed with no item id — and this catalog row declares ZERO fields, so there is nothing to key on and nothing may be invented. Write withdrawn: a writable object with no primary key derives a keyless entity whose saves cannot carry a record identifier. Reads are unaffected.'
WHERE ID = '6EA3D06C-8979-4335-AC5D-64265D2BB097';

-- ── withdraw 35. ContentAreas (c) — GET|PUT /Ams/{ClientID}/ContentAreas ──
UPDATE [__mj].IntegrationObject
SET SupportsWrite = 0,
    Description   = N'Return a content area. No scalar identifier exists: the DTO is { AreaType, VersionID, Revert, VersionLabel, VersionNotes, Publish, ContentArea } where ContentArea is a nested object, and VersionID is a version, not an identity. Write withdrawn: a writable object with no primary key derives a keyless entity whose saves cannot carry a record identifier. Reads are unaffected.'
WHERE ID = '513CA6D9-6F85-4D85-97BD-675A4F4EF7A7';

-- ── withdraw 36. Auth (d) — /auth, /authenticate (ServiceStack), /Ams/Authenticate ──
UPDATE [__mj].IntegrationObject
SET SupportsWrite = 0,
    Description   = N'The session-bootstrap exchange the connector already performs internally (GetSession): credentials in, SessionId/BearerToken out. It is not on the /Ams/{ClientID}/ record surface and is not a record. Write withdrawn: a writable object with no primary key derives a keyless entity whose saves cannot carry a record identifier. Reads are unaffected.'
WHERE ID = 'F1C63B99-96CE-45EA-AE2E-9045DF46D473';

-- ── withdraw 37. GetToken (d) — POST /OAuth/GetToken ──
UPDATE [__mj].IntegrationObject
SET SupportsWrite = 0,
    Description   = N'Returns the access token for either an authorization code or refresh token. An OAuth token exchange under /OAuth/, outside the /Ams/{ClientID}/ record surface entirely; its fields are grant parameters and the issued token. Write withdrawn: a writable object with no primary key derives a keyless entity whose saves cannot carry a record identifier. Reads are unaffected.'
WHERE ID = '38FB922C-B75D-475C-A061-A8F45562CF1C';

-- ── withdraw 38. GetAccessToken (d) — POST /OAuth/GetAccessToken ──
UPDATE [__mj].IntegrationObject
SET SupportsWrite = 0,
    Description   = N'Returns the access_token in exchange of AppId, AppSecret, Code/RefreshToken. Note: the key “AppSecert” has been deprecated and will be removed in a future update. An OAuth token exchange under /OAuth/, outside the /Ams/{ClientID}/ record surface entirely. Write withdrawn: a writable object with no primary key derives a keyless entity whose saves cannot carry a record identifier. Reads are unaffected.'
WHERE ID = 'CB3712CD-7FF7-4A10-A90E-E37CF376909E';

-- ── withdraw 39. OIDCGetAccessToken (d) — POST /OAuth/OIDC/GetAccessToken ──
UPDATE [__mj].IntegrationObject
SET SupportsWrite = 0,
    Description   = N'Returns the access token for either an authorization code or refresh token. An OIDC token exchange under /OAuth/OIDC/, outside the /Ams/{ClientID}/ record surface entirely. Write withdrawn: a writable object with no primary key derives a keyless entity whose saves cannot carry a record identifier. Reads are unaffected.'
WHERE ID = 'E75F9B4E-784B-426F-8972-3AD65E809EC3';

-- ── withdraw 40. MemberPasswordReset (d) — POST /Ams/MemberPasswordReset ──
UPDATE [__mj].IntegrationObject
SET SupportsWrite = 0,
    Description   = N'Send member password reset instructions to client via email. A client-less command route ("Send member password reset instructions to client via email") — there is no {ClientID} segment, so the connector’s /Ams/{ClientID}/{ObjectName} URL cannot reach it. Write withdrawn: a writable object with no primary key derives a keyless entity whose saves cannot carry a record identifier. Reads are unaffected.'
...
UPDATE [__mj].IntegrationObject
SET SupportsWrite = 0,
    Description   = N'Unblock the Card from FraudPrevention Service A fraud-prevention command on a differently-shaped route (/Ams/FraudPrevention/{ClientID}/...); CardIdentifier in, Success/Message out. Write withdrawn: a writable object with no primary key derives a keyless entity whose saves cannot carry a record identifier. Reads are unaffected.'
